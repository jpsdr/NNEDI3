...
extern "C" void computeNetwork0_SSE2(const float *input, const float *weights, unsigned char *d);
extern "C" void computeNetwork0_i16_SSE2(const float *inputf, const float *weightsf,unsigned char *d);
extern "C" void uc2f48_SSE2(const unsigned char *t, const int pitch, float *p);
extern "C" void uc2s48_SSE2(const unsigned char *t, const int pitch, float *pf);
extern "C" int processLine0_SSE2_ASM(const unsigned char *tempu, int width, unsigned char *dstp,const unsigned char *src3p, const int src_pitch);
extern "C" void extract_m8_SSE2(const unsigned char *srcp,const int stride, const int xdia, const int ydia, float *mstd, float *input);
	...
	threads(_threads), opt(_opt), fapprox(_fapprox)
{
	...
	else // using old prescreener
	{
		...
		else // use float dot products in first layer
		{
			...
			for (int j=0; j<4; ++j)
				for (int k=0; k<48; ++k)
					weights0[j*48+k] = (float)((bdata[j*48+k]-mean[j])/127.5);
